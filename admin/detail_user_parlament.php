...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($action === 'set_role') {
        ...
        if (mb_strlen($role) > 50) $role = mb_substr($role, 0, 50);

        $stmt = $conn->prepare("UPDATE users_alba_rosa_parlament SET role = ? WHERE idusers_parlament = ?");
        if ($stmt) {
            $stmt->bind_param("si", $role, $id);
            if ($stmt->execute()) redirectDetail($id, "Role uložena.", "success-message");
            else redirectDetail($id, "Nepodařilo se uložit roli.", "error-message");
        } else redirectDetail($id, "Chyba v dotazu.", "error-message");
    }
    ...
    if ($action === 'add') {
        ...
        if ($section !== '') {
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) redirectDetail($id, "Záznam byl úspěšně přidán.", "success-message");
                else redirectDetail($id, "Nepodařilo se přidat záznam.", "error-message");
            } else redirectDetail($id, "Chyba v dotazu.", "error-message");
        } else redirectDetail($id, "Musíte vybrat sekci.", "info-message");
    }
    ...
    if ($action === 'delete') {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) redirectDetail($id, "Záznam byl úspěšně odstraněn.", "success-message");
            else redirectDetail($id, "Chyba při mazání záznamu.", "error-message");
        } else redirectDetail($id, "Chyba v dotazu.", "error-message");
    }
    ...
    if ($action === 'add_class') {
        ...
        if ($id > 0 && $class_name !== '') {
            if (mb_strlen($class_name) > 50) $class_name = mb_substr($class_name, 0, 50);
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) redirectDetail($id, "Třída přidána.", "success-message");
                else redirectDetail($id, "Nepodařilo se přidat třídu.", "error-message");
            } else redirectDetail($id, "Chyba v dotazu.", "error-message");
        } else redirectDetail($id, "Vyplňte název třídy.", "info-message");
    }
    ...
    if ($action === 'delete_class') {
        ...
        if ($stmt) {
            $stmt->bind_param("i", $delete_id);
            if ($stmt->execute()) redirectDetail($id, "Třída byla odstraněna.", "success-message");
            else redirectDetail($id, "Chyba při mazání třídy.", "error-message");
        } else redirectDetail($id, "Chyba v dotazu.", "error-message");
    }
}
...
if ($stc = $conn->prepare("SELECT idclass_parlament, class_year, class_name FROM classes_alba_rosa_parlament WHERE idusers_parlament = ? ORDER BY class_year DESC, idclass_parlament DESC")) {
    ...
    $stc->execute();
    $rc = $stc->get_result();
    while ($c = $rc->fetch_assoc()) { $classes[] = $c; }
    $stc->close();
}
...
$q->execute();
$qr = $q->get_result();
while ($row = $qr->fetch_assoc()) { $notes[] = $row; }
$q->close();
?>
<!DOCTYPE html>
<html lang="cs">
<head>
    ...
    <style>
        ...
        .kpi { display:inline-flex; align-items:center; gap:8px; background: var(--chip); border:1px solid var(--border); border-radius:999px; padding:6px 10px; font-size:13px; }
        ...
        .select, textarea, input[type="text"] {
            border: 1px solid var(--border); border-radius: 10px; padding: 10px; width: 100%; background: #fff;
        }
        ...
        .badge { display:inline-block; padding:3px 8px; border-radius:999px; border:1px solid var(--border); background:var(--chip); font-size:12px; }
        ...
        .class-row .txt .year{ font-weight:700; }

        .role-badge { font-weight:600; letter-spacing:.2px; padding:4px 8px; border-radius:999px; border:1px solid <?php echo htmlspecialchars($roleBd); ?>; background: <?php echo htmlspecialchars($roleBg); ?>; color: <?php echo htmlspecialchars($roleTx); ?>; }
    </style>
</head>
<body>
    <div class="wrap">
        ...
        if (isset($_GET['message']) && isset($_GET['message_type'])) {
            ...
            $cls = 'info-message';
            if ($message_type === 'success-message') $cls = 'success-message';
            elseif ($message_type === 'error-message') $cls = 'error-message';
        ...
        <div class="head">
            ...
            <span class="role-badge"><?php echo $currentRole !== '' ? htmlspecialchars($currentRole) : '— bez role —'; ?></span>
        </div>
        ...
        <div class="grid">
            <div class="card">
                ...
                <form method="POST" class="form-row" style="display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap;">
                    <input type="hidden" name="action" value="set_role">
                    <input type="hidden" name="idusers_parlament" value="<?php echo $uid; ?>">
                    <input class="select" name="role" list="roles-list" value="<?php echo htmlspecialchars($currentRole); ?>" placeholder="Napište roli nebo vyberte…" aria-label="Zvolte nebo napište roli" style="flex:1;min-width:220px;" />
            ...
            <div class="card">
                ...
                <form method="POST" class="form-row" style="align-items:flex-end; gap:10px; display:flex; flex-wrap:wrap;">
                    <input type="hidden" name="action" value="add_class">
                    <input type="hidden" name="idusers_parlament" value="<?php echo $uid; ?>">

                    <div style="min-width:160px">
                        <label for="school_year" style="font-weight:600">Školní rok</label>
                        <input id="school_year" class="select school-year" type="text" name="school_year" inputmode="numeric"
                               placeholder="2024/2025" pattern="\\d{4}\\s*/\\s*\\d{4}" title="Zadejte ve formátu 2024/2025" required>
                    </div>

                    <div style="min-width:160px; flex:1">
                        <label for="class_name" style="font-weight:600">Třída</label>
                        <input id="class_name" class="select" type="text" name="class_name" maxlength="50" placeholder="např. 9.A" required>
                    </div>
